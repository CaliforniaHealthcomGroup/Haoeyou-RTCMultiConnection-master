...
var options = {
    key: fs.readFileSync(path.join(__dirname, resolveURL('keys/key.pem'))),
    cert: fs.readFileSync(path.join(__dirname, resolveURL('keys/cert.pem')))
};
...
function serverHandler(request, response) {
    try {
        ...
        if (filename.toLowerCase().indexOf('.eot') !== -1) {
            contentType = 'application/vnd.ms-fontobject';
        }
        if (filename.toLowerCase().indexOf('.ttf') !== -1) {
            contentType = 'application/octet-stream';
        }
        if (filename.toLowerCase().indexOf('.svg') !== -1) {
            contentType = 'image/svg+xml';
        }
        if (filename.toLowerCase().indexOf('.woff') !== -1) {
            contentType = 'application/font-woff';
        }
        if (filename.toLowerCase().indexOf('.woff2') !== -1) {
